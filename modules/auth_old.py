...
def _get_cookie_manager():
    """Cookie Manager を返す（ない場合は None）。毎回新規作成。"""
    ...
    try:
        ...
        logger.info("CookieManager initialized (prefix=tri-merger, type=%s)", type(cm).__name__)
        # CookieManagerのready状態をチェック
        if hasattr(cm, "ready"):
            logger.debug("CookieManager has ready() method, initial state: %s", cm.ready())
        return cm
...
def _write_auth_cookie(expire_days: int = _COOKIE_EXPIRE_DAYS):
    ...
    if hasattr(cm, "ready") and not cm.ready():
        logger.debug("_write_auth_cookie: CookieManager not ready yet (skip)")
        return
    ...
    try:
        ...
        logger.debug("Cookie payload set: %s", payload)
        cm.save()
        logger.info("auth cookie written: exp=%s", exp.isoformat())
    except Exception as e:
        logger.error("_write_auth_cookie failed with error: %s (type: %s)", str(e), type(e).__name__)
        import traceback
...
def _read_auth_cookie():
    ...
    # readyチェック
    if hasattr(cm, "ready") and not cm.ready():
        logger.debug("_read_auth_cookie: CookieManager not ready yet")
        return None
    try:
        logger.debug("Attempting to read cookie with key: %s", _COOKIE_KEY)
        logger.debug("CookieManager type: %s, has get: %s", type(cm).__name__, hasattr(cm, "get"))
        raw = cm.get(_COOKIE_KEY) if hasattr(cm, "get") else cm[_COOKIE_KEY]
        logger.debug("Raw cookie data: %s", raw)
    except Exception as e:
        logger.error("_read_auth_cookie read error: %s (type: %s)", str(e), type(e).__name__)
        import traceback
        logger.error("Traceback: %s", traceback.format_exc())
        raw = None
    if not raw:
...
def _clear_auth_cookie():
    ...
    # readyチェック
    if hasattr(cm, "ready") and not cm.ready():
        logger.debug("_clear_auth_cookie: CookieManager not ready yet (skip)")
        return
    try:
        ...
        logger.error("_clear_auth_cookie failed: %s (type: %s)", str(e), type(e).__name__)
        import traceback
        logger.error("Traceback: %s", traceback.format_exc())
...
def check_password():
    ...
    # ready状態を確認、但しst.stop()はst.rerun()を使う
    if cm is not None and hasattr(cm, "ready"):
        if not cm.ready():
            # 初回リクエストのみ再実行（無限ループを防ぐ）
            ...
            if not st.session_state.cookie_init_done:
                logger.debug("CookieManager not ready, rerunning once...")
                st.session_state.cookie_init_done = True
                st.rerun()
            else:
                # 2回目でもreadyでない場合はCookieなしで続行
                logger.warning("CookieManager still not ready after rerun, continuing without cookies")
        else:
            logger.debug("CookieManager is ready")
            # readyになったらフラグをリセット
            if "cookie_init_done" in st.session_state:
                st.session_state.cookie_init_done = False
